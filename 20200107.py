    ...
    for k_sample,v_sample in complex_dict.items():
        ...
        for k_id in mid_dict.keys():
            if int(mid_dict[k_id]) > 0 and int(mid_dict[k_id]) <= 10:
                with open("%s/se_v1/%s/bracken/%s.kraken.output.txt"%(cur_path,k_sample,k_sample),"r") as in_txt:
                    for line in in_txt:
                        line = line.strip().split("\t")
                        # print(line[3].split(" ")[-1])
                        if line[3].split(" ")[-1] == "(taxid %s)"%k_id:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("exclude the number of complex",usage="\npython exclude_info_complex.py -s [sample.list] -b [batch]")
