...
def exclude_seq(cur_path,complex_dict):
    ...
    tar_cmd = "cd  %s/se_v1/Infos_TMP/exclude_complex/ && tar -zcvf %s_complex.tar.gz *"%(cur_path,args.batch)
...
def exclude_aspergillus_seq(cur_path,sample_list):
    for sample in sample_list:
        ...
        subprocess.check_output(exclude_seq,shell=True)
        # get the file size
        # if os.path.getsize("%s/se_v1/Infos_TMP/exclude_complex/%s.5052.fasta"%(cur_path,sample)) == 0:
           # os.remove("%s/se_v1/Infos_TMP/exclude_complex/%s.5052.fasta"%(cur_path,sample))
        rm_file = "rm %s/se_v1/Infos_TMP/exclude_complex/tmp"%(cur_path)
        subprocess.check_output(rm_file,shell=True)
...
if __name__ == '__main__':
    ...
    if file == True:
        exclude_aspergillus_seq(cur_path, sample_list)
